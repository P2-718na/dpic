...
\begin{definition}
    ...
    Siano $Q \in \M_{n\times n}(\R)$, $R \in \M_{n\times m}(\R)$
    ...
    detta \textbf{regolatore lineare quadratico}.
    \label{def:lqr}
\end{definition}
...
\begin{equation*}
    K = R^{-1}BX
\end{equation*}
dove $X$ è la soluzione all'equazione di Riccati
\begin{equation}
        A^{\T} X + X A - X B R^{-1} B^{\T} X + Q = \b 0.
    \label{eq:riccati}
\end{equation}
La soluzione della~\eqref{eq:riccati} si può trovare numericamente in tempo polinomiale
in $n$.
%todo qui ci va una reference lol
